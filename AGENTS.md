...
> 当前事实：实验生命周期、Jupyter 00-06、训练正确性修复、结构化指标、单/双卡 Smoke、pinned CLIP Mini Run、20 epoch 完整训练、同域测试和 14 个跨数据集测试均有验证证据；标准化 CSV 结果已写入 Git 外归档，完整 checkpoint 归档仍未执行。只有存在明确提交、测试或运行证据的项目才能标记为完成。
...
当前 Notebook 实施状态以 `TASK_INDEX.md` 为准；现有标准清单为：
