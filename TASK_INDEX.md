...
Current task branch: `exp/full-reproduction`

Completed scope: formal protocol, fallback node freeze and bounded throughput validation

Next task branch: merge `exp/full-reproduction`, then wait for explicit formal-training approval
...
**状态：BLOCKED by formal-training approval**
...
- 先前 formal RUN_ID 因代码或配置被修复取代，均按不可复用规则标记 `aborted`；当前 planned RUN_ID 绑定提交 `8c11418` 并通过路径、checksums 和 30 GiB 预算验证。合并 PR 后仍需以最终 `main` 提交重新编号冻结，不能直接执行旧 manifest。

下一步：提交并审查 `exp/full-reproduction`，合并后重新生成唯一 planned RUN_ID/config/manifest 并复核 GPU、空间、JSON、CLIP 和 Git clean。只有用户明确批准 20 epoch 正式训练后才执行保存的命令；训练完成并冻结 best SHA-256 后，T4.5 才能开始。
